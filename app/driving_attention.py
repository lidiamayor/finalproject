...
class DrivingAttention:
    ...
    def __init__(self, model_path, database_path, data_file, data_frame_file, data_status, threshold):
    ...
    @st.dialog("I don't recognize you. Enter your personal data")
    def input(self):
        self.name = st.text_input("Name")
        age = st.text_input("Age")
        gender = st.text_input("Gender (M o F)")
        print('introduce data')
        faceid = True
        if st.button('Submit'):
            print('boton pulsado')
            # Guardar el nuevo rostro
            self.people_data = self.save_new_face(age, gender)
            faceid = False
            st.rerun()       
        print('faceid de input: ', faceid)
        return faceid
    ...
    def faceid_init(self):#frame, known_face_encodings, known_face_ids, data_file, database_path, data_frame_file, people_data):
        ...
        for (top, right, bottom, left), self.face_encoding in zip(face_locations, face_encodings):
            ...
            if True in matches:
                ...
                print('llamada a input')             
                faceid = self.input()
                if faceid == None: 
                    faceid=True
                print('faceid despues de input', faceid)
        ...
        st.title(self.name)
        print('faceid que va a devolver', faceid)
        return faceid
    ...
    def botton_status(self, detections):
        # Inicializamos los tres botones al principio con color apagado (gris)
        left, middle, right = st.columns(3)

        if detections == "focus":
            left.button("Focus", use_container_width=True, key="focus", disabled=False, on_click=None)
            middle.button("Distracted", use_container_width=True, key="distracted", disabled=True)
            right.button("Tired", use_container_width=True, key="tired", disabled=True)
        elif detections == "distracted":
            left.button("Focus", use_container_width=True, key="focus", disabled=True)
            middle.button("Distracted", use_container_width=True, key="distracted", disabled=False, on_click=None)
            right.button("Tired", use_container_width=True, key="tired", disabled=True)
        elif detections == "tired":
            left.button("Focus", use_container_width=True, key="focus", disabled=True)
            middle.button("Distracted", use_container_width=True, key="distracted", disabled=True)
            right.button("Tired", use_container_width=True, key="tired", disabled=False, on_click=None)
